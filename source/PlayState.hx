...
class PlayState extends MusicBeatState
{
	...
	public var RATING_X:Float = 400;
	public var RATING_Y:Float = 130;
	public var NUM_SCORE_X:Float = 400;
	public var NUM_SCORE_Y:Float = 130;
	...
	public var camNoteMoveOffset:Int = 14;
	...
	public var ratingOffset:Array<Float> = null;
	public var numScoreOffset:Array<Float> = null;
	...
	override public function create()
	{
		...
		if(stageData == null) { //Stage couldn't be found, create a dummy stage for preventing a crash
			stageData = {
				...
				rating_offsets: [400, 0],
				numScore_offsets: [400, 0]
			};
		}
		...
		RATING_X = stageData.rating_offsets[0];
		RATING_Y = stageData.rating_offsets[1];
		NUM_SCORE_X = stageData.numScore_offsets[0];
		NUM_SCORE_Y = stageData.numScore_offsets[1];
		...
		ratingOffset = stageData.rating_offsets;
		if(ratingOffset == null)
			ratingOffset = [400, 0];

		numScoreOffset = stageData.numScore_offsets;
		if(numScoreOffset == null)
			numScoreOffset = [400, 0];
		...
		switch (curStage)
		{
			...
			case 'spooky': //Week 2
				...
				//PRECACHE SOUNDS
				precacheList.set('stage-specific/thunder_1', 'sound');
				precacheList.set('stage-specific/thunder_2', 'sound');
			...
			case 'mall': //Week 5 - Cocoa, Eggnog
				var bg:BGSprite = new BGSprite('backgrounds/mall/bgWalls', -1000, -500, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				if(!ClientPrefs.lowQuality) {
					upperBoppers = new BGSprite('backgrounds/mall/upperBop', -240, -90, 0.33, 0.33, ['Upper Crowd Bob']);
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 0.85));
					upperBoppers.updateHitbox();
					add(upperBoppers);

					var bgEscalator:BGSprite = new BGSprite('backgrounds/mall/bgEscalator', -1100, -600, 0.3, 0.3);
					bgEscalator.setGraphicSize(Std.int(bgEscalator.width * 0.9));
					bgEscalator.updateHitbox();
					add(bgEscalator);
				}

				var tree:BGSprite = new BGSprite('backgrounds/mall/christmasTree', 370, -250, 0.40, 0.40);
				add(tree);

				bottomBoppers = new BGSprite('backgrounds/mall/bottomBop', -300, 140, 0.9, 0.9, ['Bottom Level Boppers Idle']);
				bottomBoppers.animation.addByPrefix('hey', 'Bottom Level Boppers HEY', 24, false);
				bottomBoppers.setGraphicSize(Std.int(bottomBoppers.width * 1));
				bottomBoppers.updateHitbox();
				add(bottomBoppers);

				var fgSnow:BGSprite = new BGSprite('backgrounds/mall/fgSnow', -600, 700);
				add(fgSnow);

				santa = new BGSprite('backgrounds/mall/santa', -840, 150, 1, 1, ['santa idle in fear']);
				add(santa);
				precacheList.set('Lights_Shut_off', 'sound');

			case 'mallEvil': //Week 5 - Winter Horrorland
				var bg:BGSprite = new BGSprite('backgrounds/mall/evilBG', -400, -500, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				var evilTree:BGSprite = new BGSprite('backgrounds/mall/evilTree', 300, -300, 0.2, 0.2);
				add(evilTree);

				var evilSnow:BGSprite = new BGSprite('backgrounds/mall/evilSnow', -200, 700);
				add(evilSnow);
			...
			case 'school': //Week 6 - Senpai, Roses
				...
				var bgSky:BGSprite = new BGSprite('backgrounds/school/weebSky', 0, 0, 0.1, 0.1);
				add(bgSky);
				bgSky.antialiasing = false;

				var repositionShit = -200;

				var bgSchool:BGSprite = new BGSprite('backgrounds/school/weebSchool', repositionShit, 0, 0.6, 0.90);
				add(bgSchool);
				bgSchool.antialiasing = false;

				var bgStreet:BGSprite = new BGSprite('backgrounds/school/weebStreet', repositionShit, 0, 0.95, 0.95);
				add(bgStreet);
				bgStreet.antialiasing = false;

				var widShit = Std.int(bgSky.width * 6);
				if(!ClientPrefs.lowQuality) {
					var fgTrees:BGSprite = new BGSprite('backgrounds/school/weebTreesBack', repositionShit + 170, 130, 0.9, 0.9);
				...
				if(!ClientPrefs.lowQuality) {
					var treeLeaves:BGSprite = new BGSprite('backgrounds/school/petals', repositionShit, -40, 0.85, 0.85, ['PETALS ALL'], true);
		...
		if (creditsText)
		{
			creditsWatermark = new FlxText(4, 25, 0, credits, 16);
		...
		Lib.application.window.title = "Friday Night Funkin' - " + SONG.song + " (" + storyDifficultyText + ")";
		#end
	...
	public function updateScore(miss:Bool = false)
	{
		...
		/*
		scoreTxt.text += ' | Accuracy: ${Highscore.floorDecimal(ratingPercent * 100, 2)}%$ratingFC';
		scoreTxt.text += ' | Misses: $songMisses${ClientPrefs.lateDamage ? "" : ' (${songMisses + shits})'}';
								...
								if(dancers[i].x < FlxG.width * 1.5 && limoLight.x > (370 * i) + 170) {
									switch(i) {
										case 0 | 3:
											...
											var particle:BGSprite = new BGSprite('backgrounds/limo/gore/noooooo', dancers[i].x + 200, dancers[i].y, 0.4, 0.4, ['hench leg spin' + diffStr + 'PINK'], false);
											grpLimoParticles.add(particle);
											var particle:BGSprite = new BGSprite('backgrounds/limo/gore/noooooo', dancers[i].x + 160, dancers[i].y + 200, 0.4, 0.4, ['hench arm spin' + diffStr + 'PINK'], false);
											grpLimoParticles.add(particle);
											var particle:BGSprite = new BGSprite('backgrounds/limo/gore/noooooo', dancers[i].x, dancers[i].y + 50, 0.4, 0.4, ['hench head spin' + diffStr + 'PINK'], false);
											grpLimoParticles.add(particle);

											var particle:BGSprite = new BGSprite('backgrounds/limo/gore/stupidBlood', dancers[i].x - 110, dancers[i].y + 20, 0.4, 0.4, ['blood'], false);
											particle.flipX = true;
											particle.angle = -57.5;
											grpLimoParticles.add(particle);
										case 1:
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		var noteDiffKade:Float = -(note.strumTime - Conductor.songPosition);
		//trace(noteDiff, ' ' + Math.abs(note.strumTime - Conductor.songPosition));
		var wife:Float = EtternaFunctions.wife3(-noteDiffKade, Conductor.timeScale);
		...
		if (!ClientPrefs.advancedAccuracy)
		...
		if(ClientPrefs.scoreZoom && !cpuControlled)
		{
			...
			FlxTween.tween(scoreTxt, {"scale.x": 1, "scale.y": 1}, (Conductor.crochet / 800), {ease: FlxEase.cubeOut});
		}
		...
		for (i in seperatedScore)
		{
			...
			numScore.x += NUM_SCORE_X;
			numScore.y -= NUM_SCORE_Y;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				switch(ClientPrefs.inputsystemlmao) { // add your input systems here
					case 'Kade':
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		switch (note.noteData){
			case 0:
				dadCamMovementX = -camNoteMoveOffset;
				dadCamMovementY = 0;
			case 1:
				dadCamMovementX = 0;
				dadCamMovementY = camNoteMoveOffset;
			case 2:
				dadCamMovementX = 0;
				dadCamMovementY = -camNoteMoveOffset;
			case 3:
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			switch (note.noteData){
				case 0:
					bfCamMovementX = camNoteMoveOffset;
					bfCamMovementY = 0;
				case 1:
					bfCamMovementX = 0;
					bfCamMovementY = camNoteMoveOffset;
				case 2:
					bfCamMovementX = 0;
					bfCamMovementY = -camNoteMoveOffset;
				case 3:
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			{
				FlxG.camera.zoom += 0.035 * camZoomingMult;
